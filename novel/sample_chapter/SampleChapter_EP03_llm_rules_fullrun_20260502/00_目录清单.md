# SampleChapter 项目文件目录

## 01_总方法论与项目底层文档
- 01_AI短剧生成手册.md
- 02_爆款题材库.md
- 03_小说转AI短剧工作流.md
- 04_Log.md
- 05_当前文件清单.md

## 02_模板与通用执行文档
- 06_小说短剧骨架卡模板.md
- 07_小说改编成短剧的提示词模板.md

## 03_当前项目的原始输入文档
- 08_SampleChapter.md

## 04_当前项目的诊断与结构设计文档
- project_bible_v1.json
- 09_SampleChapter短剧适配诊断与骨架提取.md
- 10_SampleChapter短剧总纲.md
- 11_SampleChapter前3集分集设计.md
- 12_SampleChapter20集分集大纲.md
- 13_SampleChapter人物关系与角色卡.md

## 05_当前项目的剧本与镜头层文档
- episode_plan_EP03_v1.json
- 14_SampleChapter第3集剧本.md
- 14A_SampleChapter第3集完整成片剧本.md
- 15_SampleChapter第3集镜头脚本.md
- 16_SampleChapter第3集旁白字幕稿.md

## 06_当前项目的视觉与AI执行层文档
- 17_SampleChapter视觉风格与分镜方案.md
- 18_SampleChapter第3集AI生成提示词包.md
- 19_SampleChapter角色统一视觉设定包.md
- 20_SampleChapter角色海报提示词包.md
- 24_SampleChapter第3集Seedance2.0逐镜头执行表.md
- 25_SampleChapter第3集Seedance2.0最终提示词包.md
- 26_SampleChapter提示词字段映射研究.md
- 27_prompt_schema_v1.json
- 28_prompt_record_template_v1.json
- 29_prompt_episode_manifest_v1.json
- 30_model_capability_profiles_v1.json
- 31_prompt_adapter_interface_v1.md
- 35_character_lock_profiles_v1.json
- records/

## 07_当前项目的包装与生产任务单文档
- 21_SampleChapter第3集封面标题测试包.md
- 22_SampleChapter第3集角色出图清单.md
- 23_SampleChapter第3集场景出图清单.md
- 24A_SampleChapter第3集视频导演准备运行手册.md

## 质量报告
- plan_qa_report.json
