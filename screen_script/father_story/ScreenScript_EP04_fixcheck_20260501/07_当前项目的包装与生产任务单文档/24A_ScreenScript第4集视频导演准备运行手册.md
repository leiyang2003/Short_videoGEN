...
python3 scripts/run_novel_video_director.py \
  --bundle screen_script/father_story/ScreenScript_EP04_fixcheck_20260501 \
  --episode EP04 \
  --experiment-prefix screenscript_ep04_director \
  --provider openai \
...
python3 scripts/run_novel_video_director.py \
  --bundle screen_script/father_story/ScreenScript_EP04_fixcheck_20260501 \
  --episode EP04 \
  --experiment-prefix screenscript_ep04_director_check \
  --provider openai \
  --allow-data-uri-from-local \
  --prepare-only \
  --enable-high-confidence-shot-chaining \
...
## 主要输出
```text
test/screenscript_ep04_director_language/language/duration_overrides.json
test/screenscript_ep04_director_language/language/language_plan.json
test/screenscript_ep04_director_keyframes/keyframe_manifest.json
test/screenscript_ep04_director_keyframes/image_input_map.json
test/screenscript_ep04_director_shot_chain_plan.json
test/screenscript_ep04_director_director_manifest.json
```
