#!/usr/bin/env python3
"""Run the screen-script-to-video pipeline for a range of episodes."""
...
def character_image_map_errors(path: Path) -> list[str]:
    if not path.exists():
        return [f"character image map not found: {path}"]
    try:
        payload = read_json(path)
    except Exception as exc:
        return [f"character image map is not valid JSON: {path} ({exc})"]
    if not isinstance(payload, dict) or not payload:
        return [f"character image map is empty: {path}"]
    return []


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Batch-run screen2video planning, visual refs, keyframes, Seedance, assembly, and QA."
    )
    parser.add_argument("--script-dir", required=True, help="Directory containing epXXX.md screen scripts.")
    parser.add_argument("--project-name", required=True, help="Project name passed to screen2video_plan.py")
    parser.add_argument("--project-title", default="", help="Optional project title passed to screen2video_plan.py")
    parser.add_argument("--episodes", default="EP01-EP08", help="Comma/range list, e.g. EP01-EP08 or EP01,EP03")
    ...
    parser.add_argument(
        "--bundle-template",
        default="{script_parent}/{project_name}_{episode}_fullrun_v1",
        help="--out template for screen2video_plan.py under screen_script/. Fields: script_parent, project_name, episode, ep_num.",
    )
    parser.add_argument(
        "--experiment-template",
        default="{project_slug}_{episode_lower}_screen_fullrun_v1",
        help="test/ experiment prefix template. Fields: project_slug, episode_lower, episode, ep_num.",
    )
    parser.add_argument(
        "--visual-ref-root",
        default="{script_parent}/assets/visual_refs_screen_batch_v1",
        help="Shared visual reference root under screen_script by default. Fields: script_parent, project_name, episode, ep_num.",
    )
    parser.add_argument("--cover-page-dir", default="", help="Cover page dir. Defaults to <screen root>/assets/cover_page.")
    parser.add_argument("--character-image-map", default="", help="Required before director. Defaults to <screen root>/character_image_map.json.")
    ...
    parser.add_argument("--max-shots", type=int, default=18)
    parser.add_argument(
        "--selection-mode",
        choices=["rule", "llm-rules", "llm-no-rules", "compare"],
        default="llm-rules",
        help="Source parsing/selection/merging mode passed to screen2video_plan.py. Default: llm-rules.",
    )
    parser.add_argument(
        "--allow-selection-fallback",
        action="store_true",
        help="Allow screen2video_plan.py to fall back to rule selection if live LLM selection fails.",
    )
    parser.add_argument("--no-character-location-tracking", action="store_true", help="Forwarded to screen2video_plan.py for debugging.")
    parser.add_argument("--strict-location-tracking", action="store_true", help="Forwarded to screen2video_plan.py; production planning is strict by default.")
    parser.add_argument(
...
def main() -> int:
    ...
    if not script_dir.exists() or not script_dir.is_dir():
        print(f"[ERROR] script dir not found: {script_dir}", file=sys.stderr)
        return 2
    try:
    ...
    script_root = script_dir.parent if script_dir.name in {"归档", "archive", "archives"} else script_dir
    try:
    ...
    character_map = resolve_repo_path(args.character_image_map) if args.character_image_map.strip() else script_root / "character_image_map.json"
    batch: dict[str, Any] = {
    ...
    for ep_num in episodes:
        ...
        try:
            script_file = script_file_for_episode(script_dir, episode_id)
        except Exception as exc:
            print(f"[ERROR] {exc}", file=sys.stderr)
        ...
        bundle = SCREEN_ROOT / bundle_out
        experiment_prefix = format_template(args.experiment_template, ctx)
        visual_ref_root = SCREEN_ROOT / format_template(args.visual_ref_root, ctx)
        cover_page_dir = resolve_repo_path(args.cover_page_dir) if args.cover_page_dir.strip() else script_root / "assets" / "cover_page"
        ...
        if stage_enabled("director", args.through):
            map_errors = [] if args.dry_run else character_image_map_errors(character_map)
            if map_errors:
                print("[ERROR] character image map preflight failed:", file=sys.stderr)
                for error in map_errors[:20]:
                    print(f"  - {error}", file=sys.stderr)
                final_status = final_status or 2
                item["steps"].append({"name": "director_preflight", "returncode": 2, "errors": map_errors[:20]})
                if args.strict:
                    break
                continue
            cmd = [
        ...
        if records_dir.exists():
            ...
            shots = [f"SH{i:02d}" for i in range(1, args.max_shots + 1)]
        else:
            shots = []
